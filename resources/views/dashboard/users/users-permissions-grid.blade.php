            ...
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title">@lang('dashboard.permissions')</h3>
                                            <a href="{{ url('/') }}/userspermissions/create">
                        ...
                        <table id="data-grid" class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                    ...
                                    <th data-priority="2">@lang('dashboard.app_module_name')</th>
                            ...
                            <tbody>
                                @foreach ($userspermissions as $userspermission)
                                    <tr>
                                        <td>{{ $userspermission->permission_id }}</td>
                                        <td>{{ $userspermission->app_module_name }}</td>
                                        <td>{{ $userspermission->permission_name }}</td>
                                        <td>
                                            <div class="float-left">
                                                <a href="{{ url('/userspermissions' . '/' . $userspermission->permission_id) }}/edit" class="btn btn-sm bg-warning">
                                                <i class="fas fa-edit"></i>
                                                </a>
                                            </div>
                                            <div class="float-right">
                                            <form method="POST"
                                                action="{{ url('/userspermissions' . '/' . $userspermission->permission_id) }}">
                                                {{ method_field('DELETE') }}
                                                {{ csrf_field() }}
                                                <input type="hidden" name="id" value="{{ $userspermission->permission_id }}">
