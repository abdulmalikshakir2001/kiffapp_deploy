...
if (document.querySelector('.fixed-plugin')) {
  ...
  document.querySelector('body').addEventListener('click',removeShowFormFixedPlugin)
  function removeShowFormFixedPlugin(e){
    if (e.target != fixedPluginButton && e.target != fixedPluginButtonNav && e.target.closest('.fixed-plugin .card') != fixedPluginCard) {
      fixedPlugin.classList.remove('show');
    }
    if(e.target.id =='iconSidenav' || $(e.target).closest('#iconNavbarSidenav').attr('id') =='iconNavbarSidenav') {
      // alert(e.target.id)
      // alert('ok')
