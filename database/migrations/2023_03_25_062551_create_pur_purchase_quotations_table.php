...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('pur_purchase_quotations', function (Blueprint $table) {
            $table->increments('pur_quotation_id');
            $table->integer('company_id')->unsigned();
            $table->integer('supplier_id')->unsigned();
            $table->integer('pro_quotation_req_id')->unsigned();
            $table->date('delivery_date');
            $table->string('ref_num',255);
            $table->string('pur_quotation_code',255); // from this we create barcode
            $table->text('barcode');
    ...
    {
        Schema::dropIfExists('pur_purchase_quotations');
    }
};
